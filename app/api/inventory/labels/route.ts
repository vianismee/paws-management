...
// Query parameters schema for GET /api/inventory/labels
...
// Labels creation schema for POST /api/inventory/labels
...
// Labels update schema for PUT /api/inventory/labels/[id]
const UpdateLabelSchema = CreateLabelSchema.partial();

// Helper function to generate auto-code
async function generateLabelCode(type: string): Promise<string> {
  const prefix = 'LBL';

  const lastLabel = await db
    .select({ code: labels.code })
    .from(labels)
    .where(ilike(labels.code, `${prefix}-%`))
    .orderBy(desc(labels.code))
    .limit(1);

  let nextNumber = 1;
  if (lastLabel.length > 0) {
    const lastCode = lastLabel[0].code;
...
// GET /api/inventory/labels - List labels with pagination and filtering
  ...
  try {
    ...
    if (query.search) {
      whereConditions.push(
        ilike(labels.name, `%${query.search}%`)
      );
    }

    if (query.type) {
      whereConditions.push(eq(labels.type, query.type));
    }

    if (query.material) {
      whereConditions.push(eq(labels.material, query.material));
    }

    if (query.isActive !== undefined) {
      whereConditions.push(eq(labels.isActive, query.isActive));
    }
    ...
    const sortColumn = {
      ...
      code: labels.code,
      cost: labels.cost,
      currentStock: labels.currentStock,
...
// POST /api/inventory/labels - Create new label
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate auto-code
    const generatedCode = await generateLabelCode(validatedData.type);
    ...
    if (error instanceof Error && error.message.includes('duplicate key')) {
      return NextResponse.json(
        { error: 'Label with this name or code already exists' },
        { status: 409 }
      );
    }
